<template>
  <div class="flex flex-wrap gap-2.5">
    <BadgeCpu :cpu-data="deviceData?.deviceData?.cpu" />
    <BadgeNetwork :network-data="deviceData?.deviceData?.network" />
    <BadgeBattery :battery-data="deviceData?.deviceData?.battery" />
  </div>
</template>
...
const updateDeviceInfo = async () => {
  try {
    deviceData.setDeviceData(await deviceInfo.fetchDeviceInfo());
